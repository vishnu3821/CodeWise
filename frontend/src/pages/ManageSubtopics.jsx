...
import { ArrowLeft, Plus, Edit2, CheckCircle, XCircle } from 'lucide-react';
...
const ManageSubtopics = () => {
    ...
    const handleOpenModal = (sub = null) => {
        if (sub) {
            ...
            setFormData({
                name: '',
                slug: '',
                order_index: subtopics.length + 1,
                is_active: false
            });
        }
        setShowModal(true);
    ...
    return (
        <div className="manage-subtopics">
            <header className="cm-header">
                <div className="cm-header-left">
                    <button onClick={() => navigate(-1)} className="cm-back-btn">
            ...
            <main className="dashboard-content" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                    <h2>Subtopics for Topic ID: {topicId}</h2>
                ...
                <div className="cm-card" style={{ padding: 0, overflow: 'hidden' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
                            <tr>
                                ...
                                <th style={{ textAlign: 'left', padding: '1rem', color: '#64748B', fontSize: '0.85rem', fontWeight: 600 }}>SUBTOPIC NAME</th>
                        ...
                        <tbody>
                            {subtopics.map(sub => (
                                <tr key={sub.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                                    <td style={{ padding: '1rem' }}>{sub.order_index}</td>
                                    <td style={{ padding: '1rem', fontWeight: '500' }}>{sub.name}</td>
                                    <td style={{ padding: '1rem', color: '#64748B', fontFamily: 'monospace' }}>{sub.slug}</td>
                                    ...
                                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                                            <button
                                                className={`cm-action-btn ${sub.is_active ? 'danger' : 'success'}`}
                    ...
                    {subtopics.length === 0 && !loading && <div style={{ padding: '2rem', textAlign: 'center', color: '#64748B' }}>No subtopics found.</div>}
                </div>
            </main>

            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={modalMode === 'add' ? 'Add Subtopic' : 'Edit Subtopic'}>
                <form onSubmit={handleFormSubmit} style={{ padding: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1.5rem' }}>
                        ...
                        <button type="submit" className="cm-card-btn">
                            {modalMode === 'add' ? 'Create Subtopic' : 'Save Changes'}
                        </button>
                    </div>
                </form>
            </Modal>
