...
const QuestionEditor = () => {
    ...
    useEffect(() => {
        if (formData.language_id) {
            const token = localStorage.getItem('token');
            axios.get(`http://localhost:5001/api/content/languages/${formData.language_id}/topics`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    useEffect(() => {
        if (formData.topic_id) {
            const token = localStorage.getItem('token');
            axios.get(`http://localhost:5001/api/content/topics/${formData.topic_id}/subtopics`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const fetchQuestionData = async () => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`http://localhost:5001/api/content/questions/${id}`, { headers: { Authorization: `Bearer ${token}` } });
            const q = res.data;
            setFormData({
    ...
    const handleSave = async (status = null) => {
        ...
        try {
            if (isEdit) {
                await axios.put(`http://localhost:5001/api/content/questions/${id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
            } else {
                await axios.post('http://localhost:5001/api/content/questions', payload, { headers: { Authorization: `Bearer ${token}` } });
            }
