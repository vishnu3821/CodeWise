    ...
    useEffect(() => {
        const checkExam = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5001/api/training-exams/${id}`, {
    ...
    const handleStart = async () => {
        ...
        if (!showInstructions) {
            if (isPasswordRequired) {
                ...
                try {
                    const token = localStorage.getItem('token');
                    const res = await axios.post(`http://localhost:5001/api/training-exams/${id}/verify-password`,
                        { password },
                        { headers: { Authorization: `Bearer ${token}` } }
                    );
        ...
        if (instructionData) {
            try {
                const token = localStorage.getItem('token');
                await axios.post(`http://localhost:5001/api/training-exams/${id}/accept-message`, {}, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } catch (err) {
