...
const ExamList = () => {
    ...
    const fetchExams = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5001/api/content/exams', {
    ...
    const fetchLanguages = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:5001/api/content/languages', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSubmitForReview = async (examId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.patch(`http://localhost:5001/api/content/exams/${examId}/status`, {
