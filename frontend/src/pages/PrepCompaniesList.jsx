...
const PrepCompaniesList = () => {
    ...
    const fetchCompanies = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5001/api/placement-prep/companies', {
    ...
    const openEditModal = (company) => {
        ...
        const previewUrl = company.logo_path ? `http://localhost:5001/uploads/companies/${company.logo_path}` : null;
    ...
    const handleSaveCompany = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                await axios.post('http://localhost:5001/api/placement-prep/companies', data, {
                    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'multipart/form-data' }
                });
                toast.success('Company added successfully');
            } else {
                await axios.put(`http://localhost:5001/api/placement-prep/companies/${currentCompany.id}`, data, {
                    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'multipart/form-data' }
                });
    ...
    const handleDelete = async (companyId) => {
        ...
        try {
            startLoading();
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5001/api/placement-prep/companies/${companyId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
