...
const PrepQuestionsList = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const modRes = await axios.get(`http://localhost:5001/api/placement-prep/companies/${companyId}/modules`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const mod = modRes.data.find(m => m.id === parseInt(moduleId));
            setCurrentModuleObj(mod);

            const qRes = await axios.get(`http://localhost:5001/api/placement-prep/companies/${compRes.data.id}/questions?moduleId=${moduleId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                await axios.post(`http://localhost:5001/api/placement-prep/companies/${company.id}/questions`, payload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                toast.success('Question added successfully');
            } else {
                await axios.put(`http://localhost:5001/api/placement-prep/questions/${currentQuestion.id}`, payload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleDelete = async (id) => {
        ...
        try {
            startLoading();
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5001/api/placement-prep/questions/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
