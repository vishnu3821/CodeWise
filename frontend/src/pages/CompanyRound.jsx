    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const compRes = await axios.get(`http://localhost:5001/api/placement-prep/public/companies/${companyId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setCompanyDetails(compRes.data);

                const modRes = await axios.get(`http://localhost:5001/api/placement-prep/public/companies/${companyId}/modules`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                const qRes = await axios.get(`http://localhost:5001/api/placement-prep/public/companies/${companyId}/questions?moduleId=${roundId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
