...
const QuestionDetail = () => {
        ...
        const fetchQuestionAndDraft = async () => {
            try {
                ...
                if (!readOnly) {
                    ...
                    if (user) {
                        try {
                            const draftRes = await axios.get(`http://localhost:5001/api/questions/${questionId}/draft`, {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            if (user && code) {
                try {
                    await axios.post(`http://localhost:5001/api/questions/${questionId}/draft`, {
                        userId: user.id,
                        language: slug || 'c',
                        code: code
                    });
    ...
    const handleRun = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:5001/api/code/run-cases', {
                code,
